...
def input_demand_sd(col, df, key):
    ...
    st.session_state["demand_sd"] = col.number_input(
        "Specify Demand Standard Deviation", value=sd, step=0.1, key=key
    )
...
def input_avg_lead_time(col, df, key):
    ...
    st.session_state["avg_lead_time"] = col.number_input(
        f"Specify Average Lead Time ({time_unit})", value=value, key=key
    )
...
def input_sd_lead_time(col, df):
    ...
    st.session_state["sd_lead_time"] = col.number_input(
        f"Specify Lead Time Std Dev ({time_unit})", value=value
    )
...
def input_holding_cost(col, key):
    st.session_state["holding_cost"] = col.number_input(
        "Specify Holding cost / unit / year (H)", value=0.20, key=key
    )


# Function to create input field for stockout cost per unit
def input_stockout_cost(col, key):
    st.session_state["stockout_cost"] = col.number_input(
        "Specify Stockout cost per unit (p)", value=0.20, key=key
    )
